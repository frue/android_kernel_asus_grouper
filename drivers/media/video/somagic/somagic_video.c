                                       ...
                                       char *name, v4l2_std_id norm)
{
  ...
  if (vdev == NULL) {
	...
	*vdev = *vdev_template;
//vdev->dev.release = video_device_release;
printk(KERN_ERR "somagic::%s: video_device_release= %d\n",__func__,vdev->dev.release);
...
printk(KERN_ERR "somagic::%s: Device Init Failed!\n",somagic->video.nr);
		goto err_exit;
	}
...
//somagic->video.vdev->dev.release = &somagic_vfl_release;
printk(KERN_ERR "somagic::%s: video_device_release= %d\n",__func__,somagic->video.vdev->dev.release);
	if (video_register_device(somagic->video.vdev, VFL_TYPE_GRABBER, video_nr) < 0) {
printk(KERN_ERR "somagic::%d: Could not register the device!\n",somagic->video.nr);
		goto err_exit;
	}
